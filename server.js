...
app.post('/api/posts', async (req, res) => {
    try {
        ...
        const u = await User.findOne({ username });
        if (u && u.isMuted) {
            return res.status(403).json({ success: false, message: "Apnar account mute kora ache." });
        }
        if (u && u.isSuspended && u.suspendedUntil && u.suspendedUntil > new Date()) {
            return res.status(403).json({ success: false, message: "Apnar account suspend kora ache." });
        }
        const post = new Post({ username, content, media: media || "", mediaType: mediaType || "text", poll: poll || null });
        await post.save();
        res.status(201).json({ success: true, post });
    } catch (e) { res.status(500).json({ success: false, message: "Post create failed" }); }
});
...
app.get('/api/posts', async (req, res) => {
    try {
        ...
        res.json({ success: true, posts });
    } catch (e) { res.status(500).json({ success: false, message: "Failed to fetch posts" }); }
});
...
app.get('/api/posts/user/:username', async (req, res) => {
    try {
        ...
        res.json({ success: true, posts });
    } catch (e) { res.status(500).json({ success: false, message: "Failed to fetch user posts" }); }
});
...
app.post('/api/posts/like', async (req, res) => {
    ...
    } catch (e) { res.status(500).json({ success: false, message: "Like failed" }); }
});
...
app.post('/api/posts/comment', async (req, res) => {
    try {
        ...
        const u = await User.findOne({ username });
        if (u && u.isMuted) return res.status(403).json({ success: false, message: "Apni comment korte parben na (Muted)." });
        const post = await Post.findById(postId);
        if (!post) return res.status(404).json({ success: false, message: "Post nai" });
    ...
    } catch (e) { res.status(500).json({ success: false, message: "Comment failed" }); }
});
...
app.delete('/api/posts/:id', async (req, res) => {
    try {
        ...
        if (!post) return res.status(404).json({ success: false, message: "Post nai" });
        if (post.username !== username) return res.status(403).json({ success: false, message: "Nijer post chara delete করা jabe na" });
...
app.post('/api/stories', async (req, res) => {
    try {
        ...
        const story = new Story({ username, media, mediaType: mediaType || "image", text: text || "", expiresAt });
        await story.save();
...
app.get('/api/stories', async (req, res) => {
    ...
    } catch (e) { res.status(500).json({ success: false, message: "Failed to fetch stories" }); }
});
...
app.get('/api/rooms/:username', async (req, res) => {
    ...
    } catch (e) { res.status(500).json({ success: false, message: "Failed to fetch rooms" }); }
});
...
app.get('/api/messages/:roomId', async (req, res) => {
    ...
    } catch (e) { res.status(500).json({ success: false, message: "Failed to fetch messages" }); }
});
...
app.post('/api/messages', async (req, res) => {
    ...
    } catch (e) { res.status(500).json({ success: false, message: "Message send failed" }); }
});
